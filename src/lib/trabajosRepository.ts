...
function toTrabajo(row: TrabajoRow): Trabajo {
  return {
    ...
    costoMateriales: Number(row.costo_materiales),
    costoExtra: Number(row.costo_extra),
...
function buildTrabajoUpdatePayload(data: Partial<Trabajo>) {
  ...
  if (data.precioCobrado !== undefined) payload.precio_cobrado = data.precioCobrado;
  if (data.costoMateriales !== undefined) payload.costo_materiales = data.costoMateriales;
  if (data.costoExtra !== undefined) payload.costo_extra = data.costoExtra;
  if (data.fechaEntrega !== undefined) payload.fecha_entrega = data.fechaEntrega;
...
export async function createTrabajoCompleto(data: NewTrabajoInput): Promise<string> {
  ...
  const { data: trabajoRaw, error: trabajoError } = await client
    ...
    .insert({
      ...
      que_falta: "",
      notas: "",
      precio_cobrado: data.precioCobrado,
      costo_materiales: data.costoMateriales,
      costo_extra: data.costoExtra,
      fecha_entrega: null,
    })
    .select("id")
    .single();
